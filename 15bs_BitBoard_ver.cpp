//g++ -O2 -std=c++11 15bs_BitBoard_ver.cpp -o 15bs_BitBoard_ver
...
char operation(char board[ROW][COL], ll movei[(TRN/21)+1],char pos[ROW * COL],ll* bboard) {   
...
unsigned char MH_EV(ll goalboard,ll bboard,bool cut,char pos[ROW*COL]){
...
int BEAM_SEARCH(ll goalboard,ll bboard,char board[ROW][COL],int TURN,bool cut,char pos[ROW*COL]) {
if(MH_EV(goalboard,bboard,cut,pos)==0){return 0;}    
vector<node>dque;
node n0;
...
char zero_pos=operation(temp_board, temp.ans,temp_pos,&tmp_bboard);  
for (int j = 0; j < 4; j++) {
node cand = temp;
...
pos2[nA]=pos2[nB];
pos2[nB]=abb;    
cand.score=MH_EV(goalboard,board2,cut,pos2);   
cand.prev=j;
fff[(4 * k) + j] = cand;
}
else{
...
int p=BEAM_SEARCH(goalboard,bboard,board,8,true,pos);
if(p>0){
    cout<<bestans;
    return 0;
}    
...
//printf("path=%d\n",BEAM_SEARCH2(board));
//printf("path=%d\n",BEAM_SEARCH(goalboard,bboard,board,TRN,false,pos));
BEAM_SEARCH(goalboard,bboard,board,TRN,false,pos);
cout<<bestans;
